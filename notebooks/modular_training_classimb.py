...
from GradMatch.selectionstrategies.supervisedlearning import OMPGradMatchStrategy, GLISTERStrategy, RandomStrategy, CRAIGStrategy
from GradMatch.utils.models import ResNet18, MnistNet, TwoLayerNet
...
def create_model(name, num_cls, device, dim=0, hidden_size=50):
  if name == 'ResNet18':
     model = ResNet18(num_cls)
  elif name == 'MnistNet':
     model = MnistNet()
  elif name == 'TwoLayerNet':
      model = TwoLayerNet(dim, num_cls, hidden_size)
...
def train_model(num_epochs, dataset_name, datadir, feature, model_name, fraction, select_every, learning_rate, run, device, strategy):
    ...
    print_every = 3
    all_logs_dir = 'results/' + strategy + '/' + dataset_name + '/' + feature +'/' + str(fraction) + '/' + str(select_every) + '/' + str(run)
    print(all_logs_dir)
    ...
    #Model Creation
    model = create_model(model_name, num_cls, device, dim=M, hidden_size=50)
    model1 = create_model(model_name, num_cls, device, dim=M, hidden_size=50)
    #Loss Functions
    ...
    for i in range(num_epochs):
        ...
        if strategy in ['Random-Online']:
            ...
            pass
        elif (strategy in ['GLISTER', 'GradMatch', 'GradMatchPB']) and (((i+1) % select_every) == 0):
            start_time = time.time()
    ...
    print("Total time taken by " + strategy + " = " + str(omp_cum_timing[-1]*3600))
    logfile.close()

"""#Training Arguments"""

datadir = '../../data/sklearn-digits'
data_name = 'sklearn-digits'
fraction = float(0.05)
num_epochs = int(200)
select_every = int(20)
feature = 'classimb'
